...
const AppMenu = ({ authData, configData, setLogout, groupData, boardData, expand, socketData, isTest , setIsSearch}) => {
  ...
  const menuRender = <Collapse navbar
    className={configData.dc_navConfig.navbarButtonPosition === 'center' ? `justify-content-md-center` : ''}
    isOpen={isOpen} >
    ...
    <Nav navbar className={configData.dc_navConfig.navbarButtonPosition === 'right' ? 'ml-auto' : configData.dc_navConfig.navbarButtonPosition === 'left' ? 'mr-auto' : ''}>
      ...
      {configData.dc_navgation.map((item, i) => {
        if (item.type === 'nav-item') {
          return (
            <NavItem key={i} className={` cursor-pointer ${configData.dc_navConfig.navItemPaddingY} ${router.asPath === item.to ? 'nav-focus' : null}`}
              onClick={(e) => {
                e.preventDefault()
      ...
      <AccountsMenuTemplate authData={authData} configData={configData} setLogout={setLogout} socketData={socketData} router={router}/>
  ...
  return (
    <div className='position-relative'>
      ...
      <NavBarDiv isFixed={isFixed}
        className={`navbar navbar-expand-lg d-none d-md-block ${configData.dc_navConfig.navbarPaddingY} ${configData.dc_navConfig.isScrollMovingNavFixed ? position : ''}`}
        navConfig={configData.dc_navConfig} >

        <button className='navbar-toggler' onClick={toggle}><i className='navbar-toggler-icon' /></button>
        {menuRender}
        <AdminDialogButton buttonLabel='Nav Setting' dismissableMask >
      ...
      {/* mobile top navbar */}
      <div className='navbar d-block d-md-none position-fixed top-0 w-100 bg-body py-2 border-bottom'
        style={{
          zIndex: 999,
          fontFamily: configData.dc_navConfig.navbarFontFamily
        }}>
        <div className="d-flex justify-content-between">
          <button className='btn btn-outline-link ml-2' onClick={toggle}>
      ...
      <div className='navbar d-block d-md-none position-fixed bottom-0 w-100 bg-body py-2'
      ...
      <Sidebar style={{ width: "260px" }} className='d-none-header d-block d-md-none' visible={isOpen} onHide={() => { setIsOpen(false) }}>
        {menuRender}
      </Sidebar>
  ...
  return(
    !authData.isLogin ? <>
      <NavItem className={`${configData.dc_navConfig.navItemPaddingY}`}>
        <Link href="/auth/login" ><a className="nav-link"   >로그인</a></Link>
      </NavItem >
      <NavItem className={`${configData.dc_navConfig.navItemPaddingY}`}>
        <Link href="/auth/register" ><a className="nav-link"   >회원가입</a></Link>
      </NavItem >
    </> :
      <UncontrolledDropdown inNavbar nav className={`d-none d-md-block ${configData.dc_navConfig.navItemPaddingY}`}>
        ...
        <DropdownMenu>
          <DropdownItem >  <Link href="/auth/profile" >프로필</Link></DropdownItem>
          {authData.isAdmin ? <DropdownItem >  <Link href="/admin" >관리자모드</Link></DropdownItem> : null}
